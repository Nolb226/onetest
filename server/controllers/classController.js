         ...
         await Promise.all(
            data.map(async (student, number) => {
               ...
               await newClass.createClassStudent({
                  ...
                  fullname:
                     student["Họ tên"] ||
                     student["Họ lót"] + " " + student["Tên"],
                  majorId:
                     student["chuyên ngành"] || student["Mã lớp"].slice(0, 3),
               });
            })
         );
         ...
         try {
            ...
            const newQuestions = await Promise.all(
               questionList.map(async (question) => {
                  const createdQuestion = await chapter.createQuestion(
                     question
                  );
   ...
   try {
      ...
      const newContent = await Promise.all(
         content.map(async (question) => {
            const isIn = questions.findIndex(
               (object) => object.id === question.id
            );
